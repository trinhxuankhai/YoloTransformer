...
class DeformableTransformer_Det(nn.Module):
    def __init__(
        ...
        ###
        self.query_linear = nn.Linear(9, num_ctrl_points) 
        self.roi_pool = torchvision.ops.RoIAlign(output_size=(3, 3), spatial_scale=1, sampling_ratio=-1)
        ##
    ...
    def query_embedding_generator(self, memory, proposals, spatial_shapes):
      N_, S_, C_ = memory.shape
      _cur = 0
      
      # Convert cxcywh -> x1y1x2y2
      proposals_ct = torch.zeros_like(proposals)
      proposals_ct[:, :, 0] = proposals[:, :, 0] - proposals[:, :, 2]/2.0
      proposals_ct[:, :, 1] = proposals[:, :, 1] - proposals[:, :, 3]/2.0
      proposals_ct[:, :, 2] = proposals[:, :, 0] + proposals[:, :, 2]/2.0
      proposals_ct[:, :, 3] = proposals[:, :, 1] + proposals[:, :, 3]/2.0
      proposals_ct = proposals_ct.reshape(-1, 4)
      
      batch_idxs = []
      for i in range(N_):
        batch_idxs.append(i*torch.ones((self.num_proposals, 1)))
      batch_idxs = torch.cat(batch_idxs, dim = 0).to(memory.device)
      ...
      for lvl, (H_, W_) in enumerate(spatial_shapes):
        ...
        memory_lvl = memory[:, _cur:(_cur + H_ * W_)].view(N_, -1, H_, W_)
        
        proposals_indexed = proposals_ct
        proposals_indexed[:, 0] = proposals_indexed[:, 0]*W_
        proposals_indexed[:, 1] = proposals_indexed[:, 1]*H_
        proposals_indexed[:, 2] = proposals_indexed[:, 2]*W_
        proposals_indexed[:, 3] = proposals_indexed[:, 3]*H_
        proposals_indexed = torch.cat([batch_idxs, proposals_indexed], dim = 1)
        
        # proposals feature : (bs*n_q, c, 3*3)
        proposals_ft = self.roi_pool(memory_lvl, proposals_indexed).flatten(-2)
        proposals_fts.append(proposals_ft)
        _cur += (H_ * W_)

      proposals_fts = torch.cat(proposals_fts, dim = -1)
      proposals_fts = self.query_linear(proposals_fts).reshape(N_, self.num_proposals, self.num_ctrl_points, self.d_model)
      return proposals_fts
    ...
    def forward(self, srcs, masks, query_embed):
        ...
        query_embeded_v2 = self.query_embedding_generator(output_memory, reference_points, spatial_shapes)
        #####################################
        ...
        query_embed = query_embed.unsqueeze(0).expand(bs, -1, -1, -1) + query_embeded_v2
