...
class SetCriterion(nn.Module):
    ...
    def loss_kernel(self, outputs, targets):
      gt_kernel = []
      for v in targets:
        gt_kernel.append(v["kernel"])
      gt_kernel = torch.stack(gt_kernel, dim = 0) # (N, H, W)

      kernel = outputs["pred_kernel"] # (N, H/8, W/8)
      mask = outputs["pred_mask"] # (N, H/8, W/8)

      # Interpolate gt_kernel to same size as predict kernel
      gt_kernel = F.interpolate(gt_kernel[:, None, :, :], kernel.shape[-2:], mode = 'bilinear').squeeze(1)
      #loss_dice = self.dice_loss(kernel, gt_kernel, mask)
      loss_dice = F.binary_cross_entropy(torch.sigmoid(kernel), gt_kernel)
      
      losses = {'loss_dice': 3*loss_dice}
      return losses
    ...
    def forward(self, outputs, targets):
        ...
        if 'kernel_outputs' in outputs:
            kernel_outputs = outputs['kernel_outputs']
            l_dict = self.loss_kernel(kernel_outputs, targets)
            losses.update(l_dict)
...
class SetCriterion(nn.Module):
    ...
    def loss_kernel(self, outputs, targets):
      ...
      losses = {'loss_kernel': 3*loss_dice}
      return losses
