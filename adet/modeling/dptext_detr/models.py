...
class DPText_DETR(nn.Module):
    def __init__(self, cfg, backbone):
        ...
        self.seg_head = Segmentation_Head(self.d_model)
        self.to(self.device)
    ...
    def forward(self, samples: NestedTensor):
        ...
        pred_kernel, features_share = self.seg_head(features)
        ...
        for l, feat in enumerate(features):
            ...
            srcs.append(self.input_proj[l](src) + features_share[l])
            masks.append(mask)
        ...
        pred_mask = masks[0]
        mask_shape = (int(pred_mask.shape[-2]*2), int(pred_mask.shape[-1]*2))
        pred_mask = F.interpolate(pred_mask[None].float(), size=mask_shape).to(torch.bool)[0]
        ...
        out['kernel_outputs'] = {'pred_kernel': pred_kernel, 'pred_mask': pred_mask}
        return out
...
class Segmentation_Head(nn.Module):
    def __init__(self, d_model):
        super().__init__()
        self.reduce_layer1 = RepVGGBlock(32, d_model)
        self.reduce_layer2 = RepVGGBlock(64, d_model)
        self.reduce_layer3 = RepVGGBlock(128, d_model)
        self.head_conv = RepVGGBlock(d_model*3, 64)
    ...
    def forward(self, features):
        ...
        out_shape = (int(f.shape[-2]*2), int(f.shape[-1]*2))
        f = F.interpolate(f, out_shape, mode='bilinear')

        out = self.final(f).squeeze(1)
        return out, out_lst