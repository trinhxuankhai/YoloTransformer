...
class MaskedBackbone(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        cfg = Config.fromfile("YOLOv6/configs/yolov6n_finetune.py")
        if not hasattr(cfg, 'training_mode'):
          setattr(cfg, 'training_mode', 'repvgg')
        num_classes = 1
        device = 'cuda'
        self.backbone = get_model(cfg, num_classes, device)
        self.feature_strides = [8, 16, 32]
        self.num_channels = [32, 64, 128] # yolov6n
    ...
    def forward(self, images):
        ...
        for i in range(len(features)):
            features[i] = NestedTensor(features[i], masks[i])
        return features
...
class TransformerPureDetector(nn.Module):
    ...
    def prepare_targets(self, targets, images_shape):
        _, _, h_max, w_max = images_shape
        new_targets = []
        for targets_per_image in targets:
            ...
            ### Generate kernel gt ###
            gt_kernel = torch.zeros(h_max, w_max).numpy()
            for points in raw_ctrl_points.reshape(-1, self.dptext_detr.num_ctrl_points, 2):
              gt_kernel = cv2.fillPoly(gt_kernel, pts=[points.cpu().numpy().astype(int)], color = 1)
            gt_kernel = torch.as_tensor(gt_kernel).to('cuda')
            ...
            new_targets.append(
                {"labels": gt_classes, "boxes": gt_boxes, "ctrl_points": gt_ctrl_points, 'kernel': gt_kernel}
            )
        return new_targets
